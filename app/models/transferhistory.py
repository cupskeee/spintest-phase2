...
class TransferHistory(Document):
    _id = StringField(required=True, primary_key=True)
    created_date = DateTimeField(default=datetime.now())
    amount = DecimalField(required=True, default=0, min_value=0)
    wallet = ReferenceField('Wallets', required=True)
    target = ReferenceField('Wallets', required=True)
    ...
    remarks = StringField(required=False, default="")
